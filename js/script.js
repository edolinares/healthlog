function loadHero(){ // RUNS ON LOADING PAGE
    const str = localStorage.getItem(localStg); // WE GET THE INFO OF LOCAL STORAGE
    mylog = str ? JSON.parse(str) : mylog;
    let left = document.querySelector('#left');  // DEFINE THE LEFT BOX AND ADD IMAGE
    ...
    let back = document.querySelector('#back');  // POPULATE THE BUTTONS FOR EACH TYPE OF ENTRIES (HIDDEN AT THE BEGGINING)
    inputOpt.forEach( element =>{
        button = domItem('input','','type','button','value',element);
        button.setAttribute('id',"inputOpt");
        back.appendChild(button);
    });
        //LISTENERS FOR EACH BUTTON OF ENTRY TYPE
...
function populateForm(){ // CALLED TO FILL THE FORM DEPENDING THE SELECTED ENTRY
    let log = document.querySelector('#log');
  log.innerHTML = ''; // CLEARS THE CURRENT CONTENT
    let index = 0;
    for (let i = 0; i < inputOpt.length; i++) { index = inputOpt[i] == inputOptPress ? i : index; } // IDENTIFY THE ENTRY REQUIRED
    moveH('back',50); 
    if (index > 0){ 
        let mainbox = domItem('div','','class','vertical ','id','');
        let title = domItem('h2',inputOpt[index],'class','','id','');
        mainbox.appendChild(title);  // ADD THE TITLE OF THE ENTRY
        inputForm[index].forEach (element =>{ // ADD THE TITLE OF EACH FIELD
            mainbox.appendChild(domItem('h3',element.field,'class','','id',''));
            if (element.type == 'select'){ //IN CASE IS A SELECT WE ADD THE OPTIONS
        ...
        button.setAttribute('onclick','save('+index+')'); // NOW WE ADD THE DELETEME OPTION TO EACH ROW
...
function save(index){ // FUNCTION CALLED ONCE SAVE IS CLICKED ON EACH FORM
    ...
    let formData = []; // CLEARS THE TEMPORAL ARRAY 
        ...
        inputForm[index].forEach(element =>{
            if (element.type == 'select'){  //IN CASE OF SELECT WE NEED TO FIND THE SELECTEDONE
                ...
                if (formInput.value !== ''){ // WE CHECK THE INPUTS ARE NOT BLANK IN CASE OF BLANK WE ACTIVATE ERROR TO STOP THE PROCESS
                    formData.push(formInput.value); // ELSE WE ADD THE INPUT TO OUR TEMP ARRAY
                } else {
                    formError = true; // IN CASE OF ERROR WE SEND A FLOATING PAGE TO WARN
                    notification('error','fill the form properly');
                }
            }
        });
        if (formError !== true){ // IF THERE WASN'T ERROR (EMPTY FIELDS)
            mylog.push(formData); // WE UPLOAD THE DATA FROM THE TEMP ARRAY TO OUR GLOBAL ARRAY 
            const jsonArr = JSON.stringify(mylog); 
            localStorage.setItem(localStg, jsonArr); // AND WE SEND IT TO LOCAL STORAGE
            inputForm[index].forEach(element =>{
            let formInput = document.getElementById(element.field);
...
function showLog(){ // USED TO POPULATE THE LOG
    moveH("left",50);
    const toreset = document.getElementById('log');
    toreset.innerHTML = ''; // STARTS CLEARING THE AREA
    let log = document.querySelector('#log');
    let mainbox = domItem('div','','class','vertical myLog','id','');
    for (let i = 1; i < inputOpt.length; i++) { 
        let title = domItem('h2',inputOpt[i],'class','','id',''); // FOR EACH TYPE OF FORM WE NEED TO CREATE A DIV
        let table = domItem('table','','class','myTable','id',''); // ADN THE TABLE
        ...
        if (mylog !== null) {
            mylog.forEach( element => {
                if (element[0] == current){ //IF ITS FOR THIS FORM WE START CREATING THE ROWS OF THE TABLE
                    let row = domItem('tr','','class','','id',k);
                    for (let j = 1; j < element.length; j++) {
                        row.appendChild(domItem('td',element[j],'class','','id',k));
                    }
                    let rowX = (domItem('td','','class','trash','id',k)); 
                    let icon = domItem('i','','class','uil uil-trash','id',k); // HERE WE ADD THE ICON FOR DELETE "K" IS USED TO KNOW THE ID ON THE ARRAY MYLOG
